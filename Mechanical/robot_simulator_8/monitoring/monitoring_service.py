...
from monitoring.feature_extractor import SIGNALS, FeatureExtractor, signals_from_matrix
from predictive import maintenance_engine
from predictive.event_log import EventLog
from predictive.health_model import HealthModel, load_weights, robot_health, save_weights
...
class MonitoringService(threading.Thread):
    ...
    def __init__(self):
        ...
        self._label_cache = {}
        self.health = HealthModel(0, load_weights())
    ...
    def _reset_buffer(self):
        ...
        self._signals = np.zeros((capacity, self.n, len(SIGNALS)))
        ...
        if hasattr(self, "health"):
            self.health.n = self.n
            self.health.reset()
            self.event_log.reset_state()
            self._health_results = []
            self._recommendations = []

    def pop_events(self, last_id):
        return self.event_log.since(last_id)
    ...
    def _publish(self, features, rule_results, forest_results, mode, note):
        ...
        for j in range(self.n):
            ...
            joints.append({
                "if_margin": forest["margin"] if forest else None,
                "status": status,
        ...
        if features is not None and (rule_results or forest_results):
            self._update_health(features, rule_results, forest_results, mode)
        health = self._health_results
        for j, joint in enumerate(joints):
            if j < len(health):
                joint["health"] = health[j]
                joint["recommendation"] = self._recommendations[j]
        snapshot = {
            "robot_health": robot_health(health) if health else None,
            "health_weights": dict(self.health.weights),
            "sim_time": self._sim_time,
            "last_event_id": self.event_log.events[-1]["id"] if self.event_log.events else 0,
            "joints": joints,
            "mode": mode,
            "note": note,
    ...
    def _update_health(self, features, rule_results, forest_results, mode):
        stats = None
        if self.baseline is not None and self.baseline.trained:
            _, stats = self.baseline.for_mode(mode)
        inputs = []
        extras = []
        for j in range(self.n):
            z = {}
            deviations = {}
            trend_z = None
            if rule_results and stats is not None:
                z_all = rule_results[j].get("z_all", {})
                for evidence, feature in config.HEALTH_EVIDENCE_FEATURE.items():
                    if feature in z_all:
                        z[evidence] = z_all[feature]
                    column = self.extractor.index[feature]
                    mean = stats["mean"][j, column]
                    # A percentage alone misleads on a joint with a tiny baseline
                    # (0.1 -> 0.15 N m is +50%), so it must also be significant.
                    significant = z_all.get(feature, 0.0) >= config.HEALTH_Z_START
                    if abs(mean) > 1e-9 and significant:
                        deviations[feature] = 100.0 * (features[j, column] - mean) / abs(mean)
                trend_values = []
                for feature in config.HEALTH_TREND_FEATURES:
                    column = self.extractor.index[feature]
                    std = stats["std_effective"][j, column]
                    trend_values.append((features[j, column] - stats["mean"][j, column]) / std)
                trend_z = float(np.nanmax(trend_values)) if trend_values else None
            # The forest score of a single window is noisy. It only counts as
            # evidence once the detector's own persistence rule is met; before
            # that it contributes a zero penalty (still available evidence).
            rise = None
            ratio = None
            temp_column = self.extractor.index.get("temperature_%ds_mean" % config.DETECTION_WINDOW_S)
            if temp_column is not None and np.isfinite(features[j, temp_column]):
                rise = float(features[j, temp_column]) - config.THERMAL_AMBIENT_C
                if stats is not None:
                    normal_rise = float(stats["mean"][j, temp_column]) - config.THERMAL_AMBIENT_C
                    if normal_rise > 1.0:
                        ratio = rise / normal_rise
            if_score = None
            if forest_results and forest_results[j]["status"] != NO_MODEL:
                persistent = forest_results[j]["status"] in (WARNING, CRITICAL)
                if_score = forest_results[j]["score"] if persistent else 0.0
            inputs.append({"z": z, "if_score": if_score, "trend_z": trend_z,
                           "temperature_rise": rise, "temperature_ratio": ratio})
            extras.append((z, deviations, trend_z, (rise, ratio),
                           forest_results[j]["status"] if forest_results else NO_MODEL))

        self._health_results = self.health.evaluate(self._sim_time, inputs)
        self._recommendations = []
        robot = self.robot_model.replace("\\", "/").split("/")[-1] if self.robot_model else "robot"
        for j, result in enumerate(self._health_results):
            z, deviations, trend_z, temperature, if_status = extras[j]
            rise, ratio = temperature
            result["trend_z"] = trend_z
            result["temperature_rise"] = rise
            result["temperature_ratio"] = ratio
            recommendation = maintenance_engine.recommend(result, z, trend_z, rise, ratio)
            self._recommendations.append(recommendation)
            self.event_log.update(self._sim_time, robot, j, result, recommendation, if_status,
                                  deviations, trend_z, rise, ratio)

    def _cmd_set_health_weights(self, payload):
        self.health.set_weights(payload.get("weights", {}))
        if payload.get("save"):
            path = save_weights(self.health.weights)
            self._message("Health weights saved: %s" % path)
    ...
    def _check_thermal_warmup(self):
        """A baseline collected while the motors warm up makes later data look hot."""
        column = self.extractor.index.get("temperature_%ds_trend" % config.DETECTION_WINDOW_S)
        if column is None or not self._training_features:
            return
        trends = np.array([f[:, column] for _, f in self._training_features])
        worst = float(np.nanmax(np.nanmean(trends, axis=0)))
        if worst > config.THERMAL_BASELINE_WARMUP_RATE:
            self._message("Warning: motor temperature was still rising (%.2f C/min) while the "
                          "baseline was collected. Run the robot until temperature settles, then "
                          "collect the baseline again, or temperature evidence will be unreliable."
                          % (60.0 * worst))
