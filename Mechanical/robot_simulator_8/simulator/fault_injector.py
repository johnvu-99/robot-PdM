...
FAULT_TYPES = (MOTOR_WEAKNESS, FRICTION, JOINT_STUCK, BACKLASH, SENSOR_NOISE,
               ...
               INTERMITTENT_FAULT, COOLING_FAILURE)
...
FAULT_CAPTIONS = {
    ...
    COOLING_FAILURE: "Cooling failure (overheating)",
}
...
def describe_severity(fault_type, severity):
    ...
    if fault_type == COOLING_FAILURE:
        return "temperature rise x%.1f (blocked fan or lost coolant)" % (
            1.0 + s * (config.FAULT_COOLING_MAX_SCALE - 1.0))
    return "%.0f%%" % (100.0 * s)
...
class FaultInjector(object):
    ...
    def begin_step(self, t, true_positions, targets=None, target_velocities=None):
        ...
        # Cooling: the same losses now settle at a higher temperature.
        self.cooling_scale = 1.0 + sev[COOLING_FAILURE] * (config.FAULT_COOLING_MAX_SCALE - 1.0)
    ...
    def _plant_dirty(self):
        return (np.any(self.cooling_scale != 1.0)
                or np.any(self._force_scales != 1.0) or np.any(self._applied_damping != 0.0)
