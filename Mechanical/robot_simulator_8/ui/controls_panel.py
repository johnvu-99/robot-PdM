...
class ControlsPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.fault_panel.command.connect(self.command)
        self.fault_panel.log.connect(self.log)
    ...
    def _build_logging_group(self):
        ...
        row = QHBoxLayout()
        row.addWidget(QLabel("CSV rate"))
        self.csv_rate_combo = QComboBox()
        for rate in config.CSV_RATES_HZ:
            self.csv_rate_combo.addItem("%d Hz" % rate, rate)
        self.csv_rate_combo.setCurrentIndex(
            list(config.CSV_RATES_HZ).index(config.CSV_DEFAULT_RATE_HZ))
        row.addWidget(self.csv_rate_combo, 1)
        outer.addLayout(row)

        buttons = QGridLayout()
        buttons.setSpacing(4)
        self.log_start_button = QPushButton("Start Logging")
        self.log_start_button.clicked.connect(self._on_start_logging)
        self.log_stop_button = QPushButton("Stop Logging")
        self.log_stop_button.clicked.connect(
            lambda: self.command.emit("stop_logging", {}))
        self.export_button = QPushButton("Export CSV")
        self.export_button.setToolTip(
            "Save the last %d s of full rate (240 Hz) telemetry."
            % int(config.TELEMETRY_BUFFER_SECONDS))
        self.export_button.clicked.connect(self._on_export)
        buttons.addWidget(self.log_start_button, 0, 0)
        buttons.addWidget(self.log_stop_button, 0, 1)
        buttons.addWidget(self.export_button, 1, 0, 1, 2)
        outer.addLayout(buttons)

        self.log_status = QLabel("Not logging. Files go to data/logs.")
        self.log_status.setObjectName("hint")
        self.log_status.setWordWrap(True)
        outer.addWidget(self.log_status)
        self.set_logging_status({"active": False})
        return group

    def set_logging_status(self, status):
        active = bool(status.get("active"))
        self.log_start_button.setEnabled(not active)
        self.log_stop_button.setEnabled(active)
        self.csv_rate_combo.setEnabled(not active)
        if active:
            self.log_status.setText("Logging %d Hz: %s\n%d rows, %.1f MB" % (
                status.get("rate", 0), os.path.basename(status.get("path", "")),
                status.get("rows", 0), status.get("bytes", 0) / 1048576.0))
        elif status.get("error"):
            self.log_status.setText("Logging stopped: %s" % status["error"])
        else:
            self.log_status.setText("Not logging. Files go to data/logs.")

    def _on_start_logging(self):
        self.command.emit("start_logging", {"rate": self.csv_rate_combo.currentData()})

    def _on_export(self):
        default = os.path.join(config.LOG_DIR, "export_%s.csv"
                               % time.strftime("%Y%m%d_%H%M%S"))
        path, _ = QFileDialog.getSaveFileName(self, "Export telemetry CSV", default,
                                              "CSV files (*.csv)")
        if path:
            if not path.lower().endswith(".csv"):
                path += ".csv"
            self.command.emit("export_csv", {"path": path})
    ...
    def on_robot_ready(self, description):
        ...
        self.fault_panel.build(description)
    ...
    def update_telemetry(self, snapshot):
        ...
        self.fault_panel.update_faults(snapshot.get("faults", []), snapshot.get("sim_time", 0.0))
...
class TelemetryPanel(QWidget):
    ...
    def __init__(self, monitoring_service=None, parent=None):
        QWidget.__init__(self, parent)
    ...
    def _build_table(self):
        ...
        if self._monitoring_service is not None:
            ...
            from ui.maintenance_panel import MaintenancePanel
            from ui.overview_panel import OverviewPanel
            from ui.rul_panel import RULPanel
            self.overview = OverviewPanel()
            self.anomaly = AnomalyPanel(self._monitoring_service)
            self.anomaly.log.connect(self.append_log)
            self.rul = RULPanel()
            self.rul.log.connect(self.append_log)
            self.maintenance = MaintenancePanel()
            self.maintenance.command.connect(self._monitoring_service.command)
            self.events = EventLogPanel(self._monitoring_service)
            self.anomaly.snapshot_ready.connect(self.overview.update_snapshot)
            self.anomaly.snapshot_ready.connect(self.maintenance.update_snapshot)
            self.anomaly.snapshot_ready.connect(lambda _snap: self.events.poll())
            self.tabs.addTab(self.overview, "Overview")
        self.tabs.addTab(self.table, "Telemetry")
        self.tabs.addTab(self.charts, "Charts")
        if self.anomaly is not None:
            self.tabs.addTab(self.anomaly, "Anomaly")
            self.tabs.addTab(self.rul, "RUL")
            self.tabs.addTab(self.maintenance, "Maintenance")
            self.tabs.addTab(self.events, "Log")
    ...
    def build_rows(self, description):
        ...
        self.charts.build(description)
        if self.anomaly is not None:
            self.anomaly.build(description)
            self.overview.build(description)
            self.maintenance.build(description)
    ...
    def update_telemetry(self, snapshot):
        ...
        accelerations = snapshot["acceleration"]
        forces = snapshot["force_magnitude"]
        moments = snapshot["moment_magnitude"]
        labels = snapshot.get("labels")
        temperatures = snapshot.get("temperature", [])
        self.joint_info.update_positions(positions)

        for row in range(self.table.rowCount()):
            if row >= len(positions):
                break
            self.table.item(row, 1).setText("%.1f" % temperatures[row] if row < len(temperatures) else "-")
            self.table.item(row, 2).setText("%+.4f" % positions[row])
            self.table.item(row, 3).setText("%+.4f" % targets[row])
            self.table.item(row, 4).setText("%+.4f" % errors[row])
            self.table.item(row, 5).setText("%+.3f" % velocities[row])
            self.table.item(row, 6).setText("%+.2f" % accelerations[row])
            self.table.item(row, 7).setText("%+.2f" % torques[row])
            self.table.item(row, 8).setText("%+.2f" % powers[row])
            self.table.item(row, 9).setText("%.1f" % forces[row])
            self.table.item(row, 10).setText("%.1f" % moments[row])
            if labels and row < len(labels):
                label = labels[row]
                fault_item = self.table.item(row, 11)
                fault_item.setText(label[0] if label[0] == "NORMAL" else "%s %.0f%%" % (label[0], 100 * label[1]))
                fault_item.setForeground(Qt.gray if label[0] == "NORMAL" else Qt.red)
                self.table.item(row, 12).setText("%.0f%% %s" % (label[2], label[3]))
    ...
    def update_stats(self, stats):
        ...
        labels["samples"].setText("%.1f s at 240 Hz" % (stats.get("samples", 0) / 240.0))
