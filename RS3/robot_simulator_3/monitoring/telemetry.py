...
class TelemetryRecorder(object):
    ...
    def record(self, timestamp, positions, targets, velocities, torques, reactions):
        ...
        from the joint force/torque sensors.
        """
        n = positions.shape[0]
    ...
    def take_chart_chunk(self):
        ...
        if not pending:
            return None
        self._chart_pending = []
        times = np.array([t for t, _ in pending], dtype=np.float64)
        data = np.stack([row for _, row in pending])
        return times, data
