...
class Robot(object):
    ...
    def __init__(self, client_id, urdf_path, base_position=(0.0, 0.0, 0.0),
        ...
        self._reactions = np.zeros((0, 6))

        # Movable joints whose force/torque sensor was enabled successfully.
        self.ft_sensor_enabled = []
    ...
    def load(self):
        ...
        self._enable_force_torque_sensors()
        if self.n > 0:
            self.reset_to(self.home_pose)
        return self.body_id

    def _enable_force_torque_sensors(self):
        """
        Reaction forces are only reported for joints with the sensor enabled.
        The sensor is passive: it reads constraint forces and does not change
        the dynamics.
        """
        enabled = []
        for joint_index in self.joint_indices:
            try:
                p.enableJointForceTorqueSensor(self.body_id, joint_index, 1,
                                               physicsClientId=self.client_id)
                enabled.append(True)
            except p.error:
                enabled.append(False)
        self.ft_sensor_enabled = enabled
    ...
    def _scan_joints(self):
        ...
        self._reactions = np.zeros((self.n, 6))
    ...
    @property
    def reactions(self):
        """(n, 6) Fx Fy Fz Mx My Mz, refreshed by get_states(). Shared buffer."""
    ...
    def describe(self):
        ...
        return {
            ...
            "ft_sensors": list(self.ft_sensor_enabled),
        }
