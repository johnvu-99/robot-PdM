...
class TelemetryPanel(QWidget):
    ...
    def update_telemetry(self, snapshot):
        ...
        for row in range(self.table.rowCount()):
            ...
            self.table.item(row, 5).setText("%+.2f" % accelerations[row])
            self.table.item(row, 6).setText("%+.2f" % torques[row])
            self.table.item(row, 7).setText("%+.2f" % powers[row])
            self.table.item(row, 8).setText("%.1f" % forces[row])
            self.table.item(row, 9).setText("%.1f" % moments[row])
            if labels and row < len(labels):
                label = labels[row]
                fault_item = self.table.item(row, 10)
                ...
                self.table.item(row, 11).setText("%.0f%% %s" % (label[2], label[3]))
