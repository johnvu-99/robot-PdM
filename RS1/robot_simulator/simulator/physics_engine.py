...
class SimulationWorker(QObject):
    ...
    def _setup(self):
        try:
            ...
            self.robot = Robot(self.client_id, config.URDF_PATH,
                               config.ROBOT_BASE_POSITION, config.USE_FIXED_BASE)
            self.robot.load()
            if self.robot.n == 0:
                self.log_message.emit("URDF loaded but contains no movable joints.")
                return False

            self.controller = JointController(self.client_id, self.robot)
            self.trajectory = TrajectoryGenerator(
                self.robot.n, self.robot.home_pose,
                self.robot.lower_limits, self.robot.upper_limits)
            ...
            self._target_velocities = np.zeros(self.robot.n)
            self._target_accelerations = np.zeros(self.robot.n)
            ...
            self.log_message.emit(
                "Loaded %s with %d movable joints."
                % (config.URDF_PATH, self.robot.n))
            self.log_message.emit(
    ...
    def _physics_step(self):
        if self.state == STATE_EMERGENCY_STOP:
            ...
            self.controller.apply(targets, velocities)
    ...
    def _cmd_set_speed(self, payload):
        self.trajectory.set_speed(payload.get("value", 1.0))

    def _cmd_set_amplitude(self, payload):
        self.trajectory.set_amplitude(payload.get("value", 0.6))
    ...
    def _cmd_set_gains(self, payload):
        self.controller.set_gains(payload.get("position_gain"),
                                  payload.get("velocity_gain"))
